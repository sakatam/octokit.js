...
// PHIL: Add indentation
// custom because I want to be able to introspect native browser objects *and* functions
function stringify(o, simple, visited, indentLevel) {
  var spaces = '', spacesMinus2 = '', json = '', i, vi, type = '', parts = [], names = [], circular = false;
  visited = visited || [];
  ...
  if (circular) {
    ...
    for (i = 0; i < o.length; i++) {
      parts.push(spaces + stringify(o[i], simple, visited, indentLevel + 1));
    }
    json += parts.join(',\n') + '\n' + spacesMinus2 + ']';
    json;
    ...
    if (o.constructor && ('Object' !== o.constructor.name)) {
      json = '<' + o.constructor.name + '>';
    } else {
      json = '{\n';
      for (i in o) {
        names.push(i);
      ...
      for (i = 0; i < names.length; i++) {
        parts.push(stringify(names[i], undefined, visited, 0) + ': ' + stringify(o[ names[i] ], simple, visited, indentLevel + 1) );
      }
      json += spaces + parts.join(',\n' + spaces) + '\n' + spacesMinus2 + '}';
    }
    ...
    for (i = 0; i < names.length; i++) {
      try {
        parts.push(names[i] + ': ' + stringify(o[names[i]], true, visited, indentLevel + 1)); // safety from max stack
      } catch (e) {
...
function run(cmd) {
  ...
  if (internalCmd) {
    ...
    try {
      ...
      // PHIL: If the response is a promise then wait to output until it finishes
      if (rawoutput.promise) {
        rawoutput
        .fail(function(err) {
          post(cmd, true, ['error', cleanse(stringify(err))]);
        })
        .done(function(res) {
          post(cmd, true, ['response', cleanse(stringify(res))]);
        });
...
function showhelp() {
  ...
  commands = [
    'Thanks for trying out this API!',
    'Above this text is a prompt where you can enter javascript; press enter and watch it go!',
    'Here are some example commands to get you started:',
    '  gh = new Octokit()',
    '  gh.getZen()',
    '  repo = gh.getRepo("philschatz", "octokit.js")',
    '  repo.getInfo()',
    '  b=repo.getDefaultBranch()',
    '  b.read("package.json")',
    '',
    'There are also some meta commands that can be run in the console:',
    '  :clear - to clear the history (accessed using cursor keys)',
    '  :history - list current session history',
    '  :help - this help page',
    '  :about',
